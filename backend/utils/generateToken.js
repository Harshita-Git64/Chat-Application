...
const jsonWebTokenMethod = (userId, res) => {
  const token = jwt.sign({ userId }, process.env.JWT_SECRET_KEY, {
    expiresIn: "90d",
  }); //first argument is payload which embedded in token, it can be multiple values and to verify the token payload is needed,second argu is secret key and third arg determines the expiry time of token.
